...
class VideoAPI:
    ...
    def generate_video_async(self, prompt, options=None):
        ...
        def run():
            try:
                ...
                with _video_tasks_lock:
                    remote_task_id = _video_tasks.get(task_id, {}).get("remote_task_id")
    ...
    def _poll_video_task(self, task_data, origin, headers, proxies, _progress_task_id=None):
        ...
        # 本地任务记录远端 id：前端刷新/人工排障只查询同一个远端任务，绝不据此重投。
        if _progress_task_id and task_id:
            with _video_tasks_lock:
                if _progress_task_id in _video_tasks:
                    _video_tasks[_progress_task_id]["remote_task_id"] = str(task_id)
