...
class InfiniteCanvasAPI:
    ...
    def unified_edit_image(self, prompt, options=None):
        """统一图片局部编辑（异步）。"""
        try:
            return self.unified.edit_image_async(prompt, options)
        except AppError as e:
            return e.to_dict()
        except Exception as e:
            return UnknownError(str(e)).to_dict()
    ...
    def load_project(self, file_path):
        return self.project.load_project(file_path)

    def reveal_project_in_folder(self, file_path):
        return self.project.reveal_project_in_folder(file_path)

    # ─────────────────────────────────────────
    # 4.3-A 个人备份（.icbackup）
    # ─────────────────────────────────────────
    def preview_backup(self, options=None):
        return self.backup.preview_backup(options)

    def export_backup(self, options=None):
        return self.backup.export_backup(options)
    ...
    def load_recent_projects(self):
        return self.settings.load_recent_projects()

    def touch_recent_project(self, path, name='', cover_path=None):
        return self.settings.touch_recent_project(path, name, cover_path)

    def remove_recent_project(self, path):
        return self.settings.remove_recent_project(path)
