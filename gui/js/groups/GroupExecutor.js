// gui/js/groups/GroupExecutor.js
...
const GroupExecutor = {
    ...
    async executeGroup(groupId) {
        ...
        if (!group || group.cardIds.length === 0) {
            if (window.Toast) Toast.show('组内没有卡片');
            return;
        }
        ...
        if (runnableCards.length === 0) {
            if (window.Toast) Toast.show('所有卡片均已绕过，无可执行项');
            return;
        }

        const levels = this._topologicalLevels(runnableCards, group);
        if (!levels) {
            if (window.Toast) Toast.show('卡片存在循环依赖，无法执行');
            return;
        }
    ...
    stopGroup(groupId) {
        ...
        // 停止所有 AI 卡的进行中请求
        AppState.ai.generatingCards.forEach((genState, cardId) => {
     ...
     * 如果是，则需要等上游先执行完再读数据。
     * @param {string} cardId - 待检查的卡片 ID
     * @param {string[]} levelIds - 当前层的所有卡片 ID
     * @param {Set<string>} completedIds - 当前层内已执行完毕的卡片 ID
     * @returns {boolean} true = 全部上游同层且都已完成
     */
    _allUpstreamReadyInLevel(cardId, levelIds, completedIds) {
     ...
     * @returns {boolean}
     */
    _hasUpstreamInLevel(cardId, levelIds) {
        const levelSet = new Set(levelIds);
     ...
     *
     * @param {string[]} levelIds - 当前层的卡片 ID 列表
     * @param {Object} state - 执行状态（stopRequested 等）
     * @param {string} groupId
     */
    async _executeLevelInOrder(levelIds, state, groupId) {
        ...
        if (remaining.size > 0) {
            ...
            if (window.Toast) Toast.show('层内存在循环依赖');
        }
    }
    ...
    // Kahn 拓扑分层
    // ─────────────────────────────────────────
    _topologicalLevels(cards, group) {
...
window.GroupExecutor = GroupExecutor;
