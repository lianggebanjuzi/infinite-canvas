...
import { promptTab } from './ui/prompt-tab';
...
async function init(): Promise<void> {
  ...
  historyDrawer.init();
  assetDrawer.init();
  // 统一资源抽屉（Phase 2）：单个胶囊入口 + 资源/历史/提示词页签；开合与页签切换由 leftCapsule 编排。
  // 各页签组件通过 setTabOpenRequest 请求「打开抽屉并切到我的页签」（如生成图自动切到历史）。
  leftCapsule.init();
  historyDrawer.setTabOpenRequest(tab => leftCapsule.openTo(tab));
  assetDrawer.setTabOpenRequest(tab => leftCapsule.openTo(tab));
  promptTab.init();
  cmdPanel.init();
  actionBar.init();
  imageEditor.init();
  ...
  window.addEventListener('icv:local-image-history', (event: Event) => {
    ...
    historyDrawer.addImage(d.src, { nodeId: d.nodeId, prompt: d.trace.prompt, model: d.trace.model,
      aspectRatio: d.trace.aspectRatio, resolution: d.trace.resolution, count: 1, outputType: 'image-edit',
      thumbnail: d.src, originalPath: d.origin?.path, originalUrl: d.origin?.url, width: d.width, height: d.height });
  });
  window.addEventListener('icv:open-results', (event: Event) => {
    const detail = (event as CustomEvent<{ nodeId?: string; index?: number }>).detail;
    if (detail?.nodeId) resultViewer.open(detail.nodeId, detail.index);
  });
