...
class CardView {
  ...
  private _contentFingerprint = new Map<string, { mainSrc: string; refStrip: string; title: string; status: string; error: string; text: string; assetState: string; isAsset: string; isOutpaint: string; emptyHint: string; sizeLabel: string; layoutLabel: string; provenance: string; splitState: string; galleryState: string }>();
  ...
  private buildCard(node: FlowNode): HTMLElement {
    ...
    el.innerHTML = `
      ...
      ${isTextGen || isTextSplit ? '' : '<div class="pcard-provenance" hidden></div>'}
      ${isTextGen || isTextSplit ? '' : `<div class="pcard-actions">
        <button class="pcard-act trace-detail" title="查看结果详情">${ICON_EXPAND}</button>
      ...
      ${(isTextGen || isTextSplit) ? '<div class="pcard-resize" title="拖拽调整大小"></div>' : '<div class="pcard-resize pcard-image-resize" title="拖拽调整图片节点布局大小"></div>'}
    `;
  ...
  private updateCard(el: HTMLElement, node: FlowNode): void {
    el.style.left = node.x + 'px';
    el.style.top = node.y + 'px';
    // 文本和图片卡均可持久化布局尺寸；图片卡是否按原图比例缩放由 imageAspectLocked 决定。
    el.style.width = (node.w ?? CARD_W) + 'px';
    ...
    if (img) {
      ...
      img.style.height = ((isTextGen || isTextSplit) ? (node.h ?? this.cardHeight(node)) : (node.h ?? this.cardHeight(node))) + 'px';
    }
    ...
    const layoutLabel = !isTextGen && !isTextSplit ? ((node.imageAspectLocked ?? true) ? '锁定比例' : '自由缩放') : '';
    ...
    const changed = !prev
      ...
      || prev.isOutpaint !== fp.isOutpaint
      || prev.emptyHint !== fp.emptyHint
      || prev.sizeLabel !== fp.sizeLabel
      || prev.layoutLabel !== fp.layoutLabel
      || prev.provenance !== fp.provenance
    ...
    if (changed) {
      ...
      if (img && this._editingNodeId !== node.id && !splitTyping) {
        ...
        const sizeHtml = !isTextGen && mainSrc && sizeLabel
          ? `<div class="pcard-size"><span>${escapeHtml(sizeLabel)} · ${escapeHtml(layoutLabel)}</span></div>`
          : '';
        if (isTextSplit) {
          ...
          const outpaintError = isOutpaint && node.status === 'fail' && node.error
            ? `<div class="outpaint-step-error"><b>扩图未完成</b><span>${escapeHtml(this._cardErrorText(node, true))}</span></div>`
            : '';
          img.innerHTML = `<div class="ph"><div class="ph-empty">${emptyContent(isOutpaint ? '已连接源图\n调整参数后运行扩图' : undefined)}${outpaintError}</div></div><div class="scan"></div>${refStrip}`;
        }
      }
    ...
    // 失败卡只显示可行动的简短说明；完整错误保留在 title / 右键「查看失败原因」，
    // 避免远端任务 ID 等技术细节把画布卡撑成难读的错误日志。
    const errEl = el.querySelector('.pcard-error') as HTMLElement | null;
    if (errEl) {
      errEl.textContent = node.status === 'fail' && node.error
        ? this._cardErrorText(node, isOutpaint)
        : '';
    }
    ...
    const provenanceEl = el.querySelector('.pcard-provenance') as HTMLElement | null;
    if (provenanceEl) {
      provenanceEl.textContent = provenance;
      provenanceEl.hidden = !provenance || (!mainSrc && !videoUrl);
    }
    ...
    // 查看详情走结果查看器：trace 为空时明确标识为旧配方不可用；双击仍保留纯大图预览。
    const act = el.querySelector('.pcard-act.trace-detail') as HTMLButtonElement | null;
    if (act) {
  ...
  private _cardErrorText(node: FlowNode, isOutpaint: boolean): string {
    const message = String(node.error || '').toLowerCase();
    if (isOutpaint) {
      if (/超时|timeout|无法确认|仍在处理|已提交/.test(message)) {
        return '等待超时，远端任务可能仍在处理';
      }
      return '扩图未完成，调整画布后可重新运行';
    }
    // 普通生成也不在卡片中显示多行服务端日志；完整信息仍在 title 中。
    return '生成未完成，查看详情后重试';
  }

  /** 返回卡片可读的来源摘要；只读取既有连线，不引入新的持久化字段。 */
  private _provenanceLabel(node: FlowNode): string {
    if (flowState.isAssetNode(node)) return '导入素材';
    const visited = new Set<string>([node.id]);
    let depth = 0;
    let frontier = flowState.getUpstreams(node.id);
    while (frontier.length > 0 && depth < 4) {
      const next: FlowNode[] = [];
      for (const candidate of frontier) {
        if (visited.has(candidate.id)) continue;
        visited.add(candidate.id);
        if (candidate.type === 'image-gen' && (candidate.imageUrl || candidate.refImages.length > 0)) {
          const name = this._provenanceSourceName(candidate);
          return depth > 0 ? `基于 ${name} · ${depth + 1} 步` : `基于 ${name}`;
        }
        next.push(...flowState.getUpstreams(candidate.id));
      }
      frontier = next;
      depth += 1;
    }
  ...
  /** 来源节点可读名称：操作步骤等通用标题退化为「上一步图片」，避免「基于 继续创作」这类绕口文案。 */
  private _provenanceSourceName(candidate: FlowNode): string {
    if (flowState.isAssetNode(candidate)) return '导入素材';
    const title = (candidate.title || '').trim();
    if (!title || title === '继续创作' || title === '扩图' || title === '素材' || title === '节点') return '上一步图片';
    return title;
  }
