...
/** 4.1-B 显式 @ 素材；持久化于节点 params，绝不靠扫描整张画布猜引用。 */
...
/** 视频文件只保留本地路径及轻量元数据，禁止把媒体内容放进项目 JSON。 */
...
interface GenerationTrace {
  ...
  /** 已获上游受理后的唯一远端任务；查询/断网恢复不得换 Key 重投。 */
  remoteTaskId?: string;
...
interface FlowNode {
  ...
  /** 图片卡布局：默认锁定原图比例；自由缩放只改变 w/h，不改图片像素。 */
...
interface StyleTransferParams {
  ...
  count: number;              // 1-4
  /** 画布内扩图步骤：保留为节点参数，因此可编辑、保存与重跑。旧节点缺省即普通绘图。 */
  mode?: 'draw' | 'outpaint';
  /** 扩图预览中原图相对目标画布中心的位置；缺省表示居中适配。 */
  outpaintPlacement?: { posX: number; posY: number; scale: number };
  ...
  mentions?: PromptMention[];  // 4.1-B：结构化 @ 素材引用
}
...
/**
 * 扩图执行参数（画布扩图步骤 / 调节弹层共用）：
 * 前端 canvas 合成白底底图（PNG dataURL）→ banana 系列模型带图补全。
 * 节点侧配置持久化在 StyleTransferParams；本对象是提交时的请求快照。
 */
...
type HistoryEntry =
  | {
      ...
      outputType: 'txt2img' | 'img2img' | 'outpaint' | 'image-edit';
